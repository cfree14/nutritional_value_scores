...
plot_lca_barplots <- function(data, country, type, factor, unit, group_yn, base_theme){
...
plot_lca_boxplots <- function(data, country, type, factor, base_theme){
  ...
  g1 <- ggplot(sdata1_ordered, aes(y=dqq_food_group, x=value, fill=food_group)) +
    stat_boxplot(geom = "errorbar", width = 0.4) + 
    geom_boxplot() +
    geom_point(data=stats1, mapping=aes(y=dqq_food_group, x=value, fill=food_group), pch=21, size=3) +
    # Labels
    labs(x=xtitle1, y="") +
    # Legend
    scale_fill_manual(name="Food group", values=food_group_colors) +
  ...
  g2 <- ggplot(sdata2_ordered, aes(y=dqq_food_group, x=value, fill=food_group)) +
    stat_boxplot(geom = "errorbar", width = 0.4) + 
    geom_boxplot() +
    geom_point(data=stats2, mapping=aes(y=dqq_food_group, x=value, fill=food_group), pch=21, size=3) +
    # Labels
    labs(x=xtitle2, y="") +
    # Legend
    scale_fill_manual(name="Food group", values=food_group_colors) +
